...
var pullsURL string = "https://api.github.com/repos/%s/%s/pulls/%s"

// GetPull performs a GET request to api.github.com/repos/user/repo/pulls/id.
// It returns the response body parsed in a struct.
func GetPull(token, user, repo, id string) (pull models.Pull, err error) {
	newReq, err := internal.NewReq("GET", fmt.Sprintf(pullsURL, user, repo, id), nil)
	if err != nil {
